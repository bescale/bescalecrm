...
const roleConfig: Record<AppRole, { label: string; icon: typeof Headphones; class: string; description: string }> = {
  agent: { label: "Agente", icon: Headphones, class: "bg-green-500/15 text-green-600", description: "Responde ao chat e assume conversas" },
  viewer: { label: "Visualizador", icon: Eye, class: "bg-gray-500/15 text-gray-500", description: "Visão limitada, apenas leitura" },
};
...
export default function ConfigEquipe() {
  ...
  async function handleSendInvite() {
    ...
    // Use Supabase edge function or direct email
    // For now, we compose a mailto link with the invite code
    const subject = encodeURIComponent("Convite para equipe - Bescale");
    const body = encodeURIComponent(
      `Olá!\n\nVocê foi convidado para participar da equipe no Bescale.\n\n` +
      `Para aceitar o convite:\n` +
      `1. Acesse: ${window.location.origin}/signup\n` +
      `2. Crie sua conta\n` +
      `3. Use o código de convite: ${inviteCode}\n\n` +
      `Seu cargo será: ${roleConfig[inviteRole].label}\n\n` +
      `Equipe Bescale`
    );

    window.open(`mailto:${inviteEmail}?subject=${subject}&body=${body}`, "_blank");

    toast.success(`Convite preparado para ${inviteEmail}`);
    setInviteEmail("");
    setShowInvite(false);
    setSending(false);
  }
  ...
  async function handleRemoveMember(member: TeamMember) {
    ...
    if (!confirm(`Deseja realmente remover ${member.full_name} da equipe?`)) return;
          ...
          <div className="space-y-3 pt-1">
            ...
            <div className="space-y-1.5">
              ...
              <div className="grid grid-cols-2 gap-2">
                {(Object.entries(roleConfig) as [AppRole, typeof roleConfig[AppRole]][]).map(
                  ([key, cfg]) => (
                    <button
                      key={key}
